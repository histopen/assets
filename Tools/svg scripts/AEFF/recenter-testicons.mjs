...
const inputArg = process.argv[2] || './testicons';
const ICONS_DIR = path.resolve(inputArg);
const OUT_DIR = path.resolve(path.join(inputArg, 'recentered'));
...
async function processFile(file){
  ...
  // find svg element, handling namespaced tags like ns0:svg
  let $svg = $('svg');
  if (!$svg.length) {
    const $all = $('*');
    const $candidates = $all.filter((i, el) => {
      const tn = (el.tagName || '').toString();
      return tn === 'svg' || tn.endsWith(':svg');
    });
    if ($candidates.length) $svg = $($candidates.get(0));
  }
  ...
  $svg.find('*').each((i,el)=>{
    const $el = $(el);
    const rawTag = (el.tagName || '').toString();
  ...
  // Apply translation directly to each element's transform (so geometry is moved
  // without introducing a wrapper group). This makes the translation visible
  // even to tools that don't resolve ancestor transforms.
  $svg.find('*').each((i, el) => {
    const $el = $(el);
    const rawTag = (el.tagName || '').toString();
    const tag = rawTag.includes(':') ? rawTag.split(':').pop() : rawTag;
    if (!['path','rect','image','circle','ellipse','polyline','polygon','line','g'].includes(tag)) return;
    const prev = $el.attr('transform') || '';
    const t = `translate(${dx} ${dy})` + (prev ? ' ' + prev : '');
    $el.attr('transform', t);
  });
